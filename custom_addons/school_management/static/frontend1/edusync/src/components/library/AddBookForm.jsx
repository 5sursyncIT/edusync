...
import { Book, User, Tag, Save, AlertCircle } from 'lucide-react';
...
const AddBookForm = ({ onClose, onBookAdded }) => {
  ...
  // Charger les auteurs et catégories au montage
  useEffect(() => {
    console.log('AddBookForm: Chargement des données...');
    fetchAuthors();
    fetchCategories();
  ...
  useEffect(() => {
    console.log('Authors state:', { authors, authorsLoading, authorsError });
    console.log('Categories state:', { categories, categoriesLoading, categoriesError });
  }, [authors, authorsLoading, authorsError, categories, categoriesLoading, categoriesError]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response && response.status === 'success') {
        ...
        onBookAdded && onBookAdded(response.data);
        onClose();
      } else {
        // Erreur dans la réponse
        setFormErrors({ submit: response?.message || 'Erreur lors de la création du livre' });
      }
    } catch (error) {
  ...
  const loading = authorsLoading || categoriesLoading;
  ...
  return (
    <Box sx={{ p: 4 }}>
      ...
      {hasError && (
        <Fade in={true}>
          <Box sx={{ mb: 3 }}>
            {(authorsError?.includes('Session') || authorsError?.includes('expir') || authorsError?.includes('authentif') ||
              categoriesError?.includes('Session') || categoriesError?.includes('expir') || categoriesError?.includes('authentif')) ? (
              <SessionHelper 
                onRetry={() => {
                  fetchAuthors();
                  fetchCategories();
